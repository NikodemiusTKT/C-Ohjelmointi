...
int tulostus(tuotetiedot *pAlku_tuotteet) {
    ...
    perror("Tuotetietojen lista on tyhjä!\n");
    return laskuri;
}
...
int lisaa_palautus(palautustiedot **pAlku_palautukset,tuotetiedot *pAlku_tuotteet, int id) {
    palautustiedot *ptr;
    /*  Tarkistus tyhjälle tuotelistalle */
    if (pAlku_tuotteet == NULL) {
        fprintf(stderr, "Palautusta ei voida lisätä, koska tuotelista on tyhjä.\n");
        return 1;
    }
...
int tulosta_kuitti(palautustiedot *pAlku_palautukset, tuotetiedot *pAlku_tuotteet, int luku) {
    ...
    if (pAlku_tuotteet != NULL && pAlku_palautukset != NULL) {
        ...
        int palautukset = laske_kaikki_palautukset(pAlku_palautukset);
        ...
        for (int i = 1; i < luku; i++) { 
            ...
            if (maara > 0) {
                ...
                char tiedot[20], tiedot2[20];
                sprintf(tiedot, "%s %.2fl", ptr->tyyppi,ptr->koko);
                sprintf(tiedot2, "pantit %d x %.2f = %.2f€", maara,ptr->pantti,ptr->pantti*maara);
                printf("%-20s\t%s\n", tiedot,tiedot2);
            }
        }
